...
#Region "Types and Constants"
    Private Const kHeightOffset As Double = 4
#End Region
#Region "Constructors"
    ''' <summary>
...
#Region "Private Interface"
    ...
    Private ReadOnly Property ReferenceBladeMeasurement(ByVal blade As UInteger) As RadiusMeasurement
        Get
            Return CType(Data, List(Of RadiusMeasurement))?.FirstOrDefault(Function(r) r.BladeId = blade And Math.Round(CType(r.Radius, Double)) = ReferenceRadius)
        End Get
    End Property
    ...
    Protected Overrides Sub ShowData()
        ...
        For i As Integer = 1 To BladeCount
            ...
            If ReferenceBladeMeasurement(b) IsNot Nothing Then
                ...
                Dim bladePosition As Double = (ReferenceAngle.Value - bladeAngle) - ((360 / BladeCount) * (ReferenceBlade.Value - ReferenceBladeMeasurement(b).BladeId.Value)) + kHeightOffset
                Dim Bladeposbase As Double = Math.Round(Math.Abs(bladePosition - kHeightOffset), 1)
                Dim p As Integer = seriesPosition.Points.AddXY("B" + b.ToString() + "=(" + Bladeposbase.ToString() + ")Deg", bladePosition)
                seriesPosition.Points(p).Color = GraphColorArray(i - 1)
            End If
        Next
        Chart1.Titles.Clear()
        Dim tile = Chart1.Titles.Add("Angular Position (Ref: Blade " + ReferenceBlade.ToString() + " at " + ReferenceRadius.ToString() + "%, " + ReferencePoint + ")")
        tile.Font = New Font("Arial", 12, FontStyle.Bold)
        Chart1.ChartAreas(0).AxisY.Title = "Angular Position (Degrees)"
        Chart1.ChartAreas(0).AxisY.Minimum = 0
        Chart1.ChartAreas(0).AxisY.Interval = 1
        Chart1.ChartAreas(0).AxisY.Maximum = kHeightOffset * 2
        Chart1.ChartAreas(0).AxisY.TitleFont = New Font("Arial", 11, FontStyle.Regular)
    End Sub
#End Region
